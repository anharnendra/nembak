...
if keys.is_file():
    with open('api.txt', 'r') as file:
        SHODAN_API_KEY=file.readline().rstrip('\kimonn')
else:
    file = open('api.txt', 'w')
    SHODAN_API_KEY = input('[*] chacking hati: ')
...
while True:
    ...
    try:
        myresults = Path("cek")
        query = input("[*] kamu yakin ingin ingin mengunduh dia? <iya/tidak>: ").lower()
        if query.startswith('iya'):
            print('')
            print('[~] Checking paswor hati: %s' % SHODAN_API_KEY)
            results = api.search('product:"Memcached" port:11211')
            print('[✓] ce hati: SUCCESS => dia dah kaga peduli ladi sama lu..')
            print('[~] Number of bots: %s' % results['total'])
            print('')
            print('[✓] ce hati: SUCCESS => tapi dia dah kaga peduli ladi sama lu..')
            saveresult = input("[*] tanem virus ke hatinya agar dia slalu keinget kamu? <iya/tidak>: ").lower()
            if saveresult.startswith('iya'):
